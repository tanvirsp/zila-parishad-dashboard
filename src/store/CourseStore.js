...
const CourseStore = create( (set) =>({
    ...
    SingleCourseRequest: async(id)=>{
        ...
        const res = await axios.get(`${import.meta.env.VITE_URL}/api/v1/course/${id}`);
        if(res.data.status === "success"){
    ...
    AddCourseRequest: async(data) =>{
        const res = await axios.post(`${import.meta.env.VITE_URL}/api/v1/course`, data);
        return res["data"];
    },

    
    UpdateCourseRequest: async(id, data) =>{
        const res = await axios.post(`${import.meta.env.VITE_URL}/api/v1/course/${id}`, data);
        return res["data"];
    },
