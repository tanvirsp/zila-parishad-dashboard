...
const SessionStore = create( (set) =>({
    ...
    SessionListRequest: async()=>{
        ...
        const res = await axios.get(`${import.meta.env.VITE_URL}/api/v1/sessions`);
    ...
    SingleSessionRequest: async(id)=>{
        ...
        const res = await axios.get(`${import.meta.env.VITE_URL}/api/v1/session/${id}`);
        if(res.data.status === "success"){
    ...
    AddSessionRequest: async(data) =>{
        const res = await axios.post(`${import.meta.env.VITE_URL}/api/v1/session`, data);
        return res["data"];
    },

    UpdateSessionRequest: async(id, data) =>{
        const res = await axios.post(`${import.meta.env.VITE_URL}/api/v1/update-session/${id}`, data);
        return res["data"];
    },
