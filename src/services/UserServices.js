...
exports.LoginUserService = async(req) =>{
    ...
    try {
       ...
       if( user && passwordIsCorrect){
            ...
            // User Token Create
            const token=EncodeToken(email, user.role, user._id.toString(),  user.instituteId.toString());
...
exports.UpdateRoleService =async(req) =>{
    try {
        const newRole = req.params.role;
        const id = req.params.id;

        const data =  await UserModel.updateOne({_id: id}, {$set: {role: newRole}});
        return {status:"success", data: data}
